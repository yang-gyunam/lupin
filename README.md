...
- 🎭 **프로세스 위장**: 시스템 프로세스로 위장하여 탐지 회피
- 🚀 **앱 자동 실행**: 업무 관련 앱을 자동으로 실행하여 정상적인 활동 패턴 생성
...
**Process Disguise** 패널에서 위장할 프로세스 이름 선택:
- `system_update` - 시스템 업데이트 서비스
- `chrome_helper` - 크롬 헬퍼 프로세스
- `WindowServer` - 윈도우 서버 프로세스
- `kernel_task` - 커널 작업
